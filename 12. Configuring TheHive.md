...
```
sudo grep -nE '^[[:space:]]*(node\.name|cluster\.initial_master_nodes):' /etc/elasticsearch/elasticsearch.yml
```

and I got the output:
```
sudo: unable to resolve host SOC-Lab-TheHive: Name or service not known
23:node.name: node-1
74:cluster.initial_master_nodes: ["node-1"]
109:cluster.initial_master_nodes: ["vultr"]
```
